...
#define BOTTOM_SPACE     40
#define BARRIER_SPACE    4  // spazio barriere


void DrawRectangle(uint16_t startX, uint16_t startY, uint16_t endX, uint16_t endY, uint16_t color)
{
...
void FillRectangle(uint16_t startX, uint16_t startY, uint16_t endX, uint16_t endY, uint16_t color){
	uint16_t y;
	for(y=startY; y<endY; y++)
		LCD_DrawLine(startX,y,endX,y, color);
}

void DrawQuoridorBoard(void) {
    uint16_t startX, startY, endX, endY;
		int i,j;
//		int infoBoxHeight=30;
		uint16_t rectHeight = 30; // Height of the rectangles
    uint16_t rectWidth = (WIDTH - 40) / 3; // Width of each rectangle, assuming 10 pixels padding
    // Draw the cells of the grid
    for ( i = 0; i < GRID_SIZE; i++) {
        for ( j = 0; j < GRID_SIZE; j++) {
            //parto on alto a sx
            startX = BARRIER_SPACE + j * (CELL_WIDTH + BARRIER_SPACE);
            startY = BARRIER_SPACE + i * (CELL_HEIGHT + BARRIER_SPACE);
            // calcolo fine di destra
		...
		//draw rectangles for barriers and timer
    startX = 10; // Starting X position for the first rectangle
    startY = HEIGHT - BOTTOM_SPACE; // Starting Y position for the rectangles

    // Draw three rectangles at the bottom
		//sx
    DrawRectangle(startX, startY-20, startX + rectWidth, startY + rectHeight, Black);
		//center
    DrawRectangle(startX + rectWidth + 10, startY-20, startX + (2 * rectWidth) + 10, startY + rectHeight, Black);
		//dx
    DrawRectangle(startX + (2 * rectWidth) + 20, startY-20, startX + (3 * rectWidth) + 20, startY + rectHeight, Black);

    // Now put the texts inside each rectangle
    GUI_Text(startX +2, startY -18, (uint8_t *) "P1 Wall", Black, White);
		//GUI_Text(startX +5, startY + 10, (uint8_t *) "   8", Black, White);
    GUI_Text(startX + 100, startY, (uint8_t *)"20s", Black, White);
    GUI_Text(startX + (2 * rectWidth) + 22, startY -18, (uint8_t *) "P2 Wall", Black, White);
		//GUI_Text(startX + (2 * rectWidth) + 25, startY +10, (uint8_t *) "   8", Black, White);
		
}
void DrawBarrierCounter(uint8_t player, uint8_t count) {
    char buffer[10];
    uint16_t startX, startY = HEIGHT - BOTTOM_SPACE; // Starting Y position for the rectangles
		uint16_t rectWidth = (WIDTH - 40) / 3;

    // Determine the X position based on the player number
    if (player == 0) {//player1
        startX = 10; // X position for Player 1 rectangle
        GUI_Text(startX + 5, startY + 10, (uint8_t *) "   ", Black, White); // Clear previous count
        sprintf(buffer, "   %d", count); // New count with spaces for alignment
        GUI_Text(startX + 5, startY + 10, (uint8_t *)buffer, Black, White); // Draw new count
    } else {//player2
        startX = 10 + (2 * rectWidth) + 20; // X position for Player 2 rectangle
        GUI_Text(startX + 5, startY + 10, (uint8_t *) "   ", Black, White); // Clear previous count
        sprintf(buffer, "   %d", count); // New count with spaces for alignment
        GUI_Text(startX + 5, startY + 10, (uint8_t *)buffer, Black, White); // Draw new count
    }
}
void DrawTurnIndicator(uint8_t currentPlayer) {
    // Draw the turn indicator based on the current player
		//0=PLAYER1 1=PLAYER2
    if (currentPlayer == 0) {
        // Draw Player 1's turn indicator
        GUI_Text(112, 262, (uint8_t *)"P1", Blue, White);
    } else if (currentPlayer == 1) {
        // Draw Player 2's turn indicator
        GUI_Text(112, 262, (uint8_t *)"P2", Red, White);
    }
}

void updateLCDTimer(uint8_t countdown) {
    char text[8];
		uint16_t startX, startY, rectWidth;
    // Set the starting X position for the timer rectangle
    startX = 10 + (WIDTH - 40) / 3 + 10; // This might be your calculated X position for the timer rectangle
    startY = HEIGHT - BOTTOM_SPACE; // Starting Y position for the rectangles
    rectWidth = (WIDTH - 40) / 3; // Width of each rectangle

    // Clear the previous time
    FillRectangle(startX + 1, startY + 1, startX + rectWidth - 1, startY + 30 - 1, White);

    // Format the new time string
    sprintf(text, "  %2ds", countdown);

    // Draw the new time
    GUI_Text(startX + 8, startY, (uint8_t *)text, Black, White); // Make sure this position matches exactly where the "19s" is being drawn
}

// Additional function to draw an empty cell
void DrawCell(uint16_t Row, uint16_t Col) {
    // Fill the cell with background color
    FillRectangle(Col+5, Row+5, Col + CELL_WIDTH-5, Row + CELL_HEIGHT-5, White); 

    // Redraw the cell border
    DrawRectangle(Col, Row, Col + CELL_WIDTH, Row + CELL_HEIGHT, Black);
}
...
{// oer scriverein un registro devo tirare giu bit e tirare su bit in tante parti
	LCD_CS(0);
	LCD_RS(0);
	LCD_RD(1);
	LCD_Send( index ); 
...
void LCD_Initialization(void)
{
	...
	LCD_Configuration();
	delay_ms(100);//delay di 100 ms, in emulazione creano problema siccome ritardano tutto in modo non necessario e non possiamo scalarle, siccome è un ciclo software
								// l unico modo di scalarle è mettere un valore diverso nella funzione
