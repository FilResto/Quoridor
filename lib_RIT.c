...
uint32_t init_RIT ( uint32_t RITInterval )
{
  ...
  LPC_SC->PCLKSEL1  |=  (1<<26);   // RIT Clock = CCLK
	LPC_SC->PCONP     |=  (1<<16);   // Enable power for RIT abilita l alimentazione del RIT, solitamente lo facevamo 
																   // col PCONP dal conf wizard, assegnamo un 1 al bit specifico PCONP per accenderlo
	...
	NVIC_EnableIRQ(RIT_IRQn);//non andiamo a dare prio, ma di default sarà uguale a 0
  return (0);
}
